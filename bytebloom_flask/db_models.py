...
class MenuItemInfo(db.Model):
    ...
    mid = db.Column(db.Integer, db.ForeignKey('MenuItem.mid'), nullable=False)
...
class Order(db.Model):
    ...
    - id (int): numeric ID of this customer's order. Autogenerated.
    ...
    def get_cost(self):
        subtotal = sum(item.item_type.price * item.count for item in self.items)
        if self.promo_code is not None:
            total = subtotal * self.promo_code.discount
        else:
            total = subtotal
        return total
...
class PromoCode(db.Model):
    ...
    - id (int): optional numeric ID. Autogenerated.
    - code (string): the text of the code, as entered into the cashier UI.
    - discount (int): the number that the cost of the order is multiplied by.
    - description (str): description of the promotion.
    - start_date (datetime.date): the start date of the promotion.
    - end_date (datetime.date): the end date of the promotion.
    - active (bool): whether the promotion is currently active or not.
    """
...
def get_current_promotions():
    ...
    return PromoCode.query.filter_by(active=True).all()

def get_removed_promotions():
    """Returns a list of inactive promotions."""
    return PromoCode.query.filter_by(active=False).all()
