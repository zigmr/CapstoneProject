...
class PromoCode(db.Model):
    ...
    - discount_multiplier (int): the number that the cost of the order is multiplied by.
    """
    __tablename__ = "PromoCode"
    code = db.Column(db.String, primary_key=True)
    discount_multiplier = db.Column(db.Float, nullable=False)

    @db.validates('discount_multiplier')
    def validate_discount(self, key, value):
        if value < 0.0:
            raise ValueError("Discount cannot reduce the cost of the order below $0.")
        elif value >= 1.0:
            raise ValueError("Discount should not increase the cost of the order.")
        else:
            return value
